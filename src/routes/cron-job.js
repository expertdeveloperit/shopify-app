...
const allfuntion = module.exports  = {
	...
	CreatePage(store){
	    ...
	    const shopRequestUrl = 'https://'+store.storeInfo.myshopify_domain+'/admin/pages.json';
	...
	insertScriptIntoTheme(store){
	    const shopRequestUrl = 'https://'+store.storeInfo.myshopify_domain+'/admin/themes.json';
	    const accessToken = store.storeInfo.accessToken ;
	    const shopRequestHeaders = {
	      'X-Shopify-Access-Token': accessToken,
	    };
	    ...
	    .then((shopResponse) => {
	        ...
	        if(themeId){
	            let url = `https://`+store.storeInfo.myshopify_domain+`/admin/themes/${themeId}/assets.json?asset[key]=layout/theme.liquid&theme_id=${themeId}`;
