...
const forwardingAddress = "https://shopify1.llank.com"; // Replace this with your HTTPS Forwarding address
...
router.get('/settings',middleware, (req, res, next) => {
    var params = req.query;
    shopifyAppModel.findOne({'storeInfo.myshopify_domain':params.shop},'status page_handle date_format',function (err, data) {
      if(err){ 
...
router.post('/settings/update',middleware, (req, res, next) => {
  ...
  shopifyAppModel.findOneAndUpdate({'storeInfo.myshopify_domain':req.query.shop},data,function (err, data) {
    if(err){
